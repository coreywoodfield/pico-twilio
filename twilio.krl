ruleset twilio {
    ...
    messages = function(account_sid, auth_token, page=0, page_size=50, to=null, from=null) {
      http:get(<<https://#{account_sid}:#{auth_token}@api.twilio.com/2010-04-01/Accounts/#{account_sid}/Messages.json?Page=#{page}&PageSize=#{page_size}>> + ((to == null) => "" | <<&To=#{to}>>) + ((from == null) => "" | <<&From=#{from}>>)).decode(){"messages"}
    }
	}
